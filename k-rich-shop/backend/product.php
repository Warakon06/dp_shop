      						...
      						<div class="card card-primary card-outline">
              					<div class="card-header">
                                    ...
                                    <div class="card-tools">
                                        <a href="#" onclick="addProduct()" class="btn btn-success"><i class="fas fa-plus-circle"></i> เพิ่ม</a>
                                    </div>
              					</div>
              					<div class="card-body">
	                				<div class="row">
                                            ...
                                            <table id="example1" class="table table-bordered table-hover table-striped">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th style="text-align: center; font-weight: bold;">คงเหลือ/หน่วย</th>
                                                        <th style="text-align: center; font-weight: bold;">&nbsp;</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <?php 
                                                        //คิวรี่เพื่อดึงข้อมูล หมวดหมู่สินค้า
                                                        $query = 'select 
                                                            a.product_id, a.name, a.price, a.amount, a.unit, 
                                                            c.photo, b.name as category_name 
                                                            from product as a 
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="text-align: center;"><img src="../img/product/<?php if (!empty($val['photo'])) { echo $val['photo']; } else { echo 'product-avarta.jpg'; } ?>" alt="" width="100px;" class="img-thumbnail"></td>
                                                        <td style="text-align: center;"><?php echo $val['category_name']; ?></td>
                                                        <td><?php echo $val['name']; ?></td>
                                                        <td style="text-align: right;"><?php echo number_format($val['price'], 2); ?></td>
                                                        <td style="text-align: center;">
                                                            <?php 
                                                                if ($val['amount']=='-1') {
                                                                    echo 'ไม่จำกัด'.'/'.$val['unit']; 
                                                                } else {
                                                                    echo $val['amount'].'/'.$val['unit']; 
                                                                }
                                                            ?>
                                                        </td>
                                                        <td>
                                                            <div class="btn-group">
                                                                <a href="#" onclick="editProduct('<?php echo $val['product_id']; ?>')" class="btn btn-warning"><i class="fas fa-edit"></i></a>
                                                                <a href="#" onclick="confirmDelete('<?php echo $val['product_id']; ?>')" class="btn btn-danger"><i class="fas fa-trash-alt"></i></a>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                    <?php } } ?>
...
<script>
    ...
    function addProduct() {
        ...
        $('#product-modal').load('add_product_modal.php', data, function(){
    ...
    function editProduct(product_id) {
        ...
        $('#product-modal').load('edit_product_modal.php', data, function(){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    data : {id_del:product_id},
                    success: function(response){
                        //console.log(response);
                       if (response=='true') {
                            top.window.location='product.php?act=del_success';
                        } else {
                            top.window.location='product.php?act=del_error';
                        }
                    }
                });
            }
