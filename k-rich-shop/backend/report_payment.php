  		...
  		<div class="content-wrapper">
    		...
    		<div class="content-header">
      			<div class="container-fluid">
        			<div class="row mb-2">
          				<div class="col-sm-6">
            				<h1 class="m-0">รายงานการชำระเงิน</h1>
          				</div>
          				<!-- /.col -->
	          			<div class="col-sm-6">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <select name="status" class="form-control select2" requierd>
                                                        ...
                                                        <option value="2">รอตรวจสอบยอดเงิน</option>
                                    ...
                                    <table id="example1" class="table table-bordered table-hover table-head-fixed text-nowrap">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="text-align: center; font-weight: bold;">ยอดสั่งซื้อ (บาท)</th>
                                        ...
                                        <tbody>
                                            <?php
                                                $query = 'select 
                                                    a.orders_id, a.payment_method, a.card_no, 
                                                    a.expire_month, a.expire_year, a.card_name, 
                                                    a.slip, a.sum_total, 
                                                    case when a.payment_method = 1 then "โอนผ่านบัญชีธนาคาร" 
                                                    when a.payment_method = 2 then "บัตรเครดิต/เดบิต" 
                                                    when a.payment_method = 3 then "สแกนคิวอาร์โค้ด" 
                                                    else "" end as payment_method_name, 
                                                    case when a.status = 1 then "ยังไม่ชำระเงิน" 
                                                    when a.status = 2 then "รอตรวจสอบยอดเงิน" 
                                                    when a.status = 3 then "ยืนยันการชำระเงิน" 
                                                    when a.status = 4 then "ยืนยันการชำระเงิน" 
                                                    when a.status = 5 then "ยืนยันการชำระเงิน" 
                                                    when a.status = 8 then "ยืนยันการชำระเงิน" 
                                                    else "" end as status_name 
                                                    from orders as a 
                                                    where 
                                                    date(a.orders_date) >= :stdate 
                                                ...
                                                if (!empty($_POST['status'])) {
                                                    if ($_POST['status']!=3) {
                                                        ...
                                                        $query .= ' and a.status not in (1, 2, 6, 7) ';
                                                    }
                                                }
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align: center;"><?php echo $value['payment_method_name']; ?></td>
                                                <td>
                                                    <?php
                                                        if ($value['payment_method']==1 or $value['payment_method']==3) { 
                                                            echo 'แนบสลิปหลักฐาน';
                                                        } else {
                                                            echo 'เลขบัตร '.$value['member_name'].'<br>'; 
                                                            echo 'หมดอายุ '.$value['expire_month'].'/'.$value['expire_year'].'<br>'; 
                                                            echo 'ผู้ถือบัตร '.$value['card_name'];
                                                        }
                                                    ?>
                                                </td>
